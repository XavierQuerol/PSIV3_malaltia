...
train_dataset = ImagesDataset(data=data, data_dir=CROPPED_PATCHES_DIR, transform=train_transform)
...
for epoch in range(num_epochs):
    ...
    model.train()
    total_loss = 0
    for batch_idx, images in enumerate(train_dataloader):  # No labels are needed
    ...
    total_loss_test = 0
    with torch.no_grad():
        for images in test_dataloader:  # No labels are needed
            images = images.to(device)
